...
3. **Execute Suite**:
   ...
   **pnpm only** — never `npm`, `yarn`, or `npx`.
   ...
   > This phase needs network access to the test MySQL instance. If your `commandExecutionPolicy: sandbox` blocks that connection, report the run as **environmentally blocked** — do not report `BLOCKED` against the code, which would misattribute an infrastructure failure to the implementation agents.
